...
func create_tables() {
	...
	execSQL("create table if not exists problem (id integer primary key, teacher_id integer, question text unique, lifetime timestamp, status integer, created_at timestamp, updated_at timestamp)")
	...
	execSQL("create table if not exists grade (id integer primary key, teacher_id integer, student_id integer, submission_id integer, score integer, code_feedback blob, comment text, status integer, created_at timestamp, updated_at timestamp, UNIQUE (teacher_id, submission_id))")
...
func init_database(db_name string) {
	...
	AddFeedbackSQL = prepare("insert into grade (teacher_id, submission_id, student_id, score, code_feedback, comment, status, created_at, updated_at) values (?, ?, ?, ?, ?, ?, ?, ?,?)")
	UpdateScoreSQL = prepare("update grade set score=?, updated_at=?, status=0 where submission_id=?")
	UpdateFeedbackSQL = prepare("update grade set code_feedback=?, comment=?,updated_at=?, status=0 where teacher_id=? and submission_id=?")
	UpdateSubmissionFeedbackGivenSQL = prepare("update submission set status=? where id=?")
	AddProblemSQL = prepare("insert into problem (teacher_id, question, lifetime, status, created_at, updated_at) values ( ?, ?, ?, ?, ?, ?)")
